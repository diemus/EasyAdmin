...
(function ($) {
    ...
    var createControls = function (that, header) {
        ...
        if (addedFilterControl) {
            ...
            header.off('change', 'select').on('change', 'select', function (event) {
                ...
                //    return;
                //}
                //注释了上面的字段，使得change事件不收searchOnEnterKey影响
    ...
    $.extend($.fn.bootstrapTable.defaults, {
        ...
        filterTemplate: {
            ...
            //使用date range
            datepicker: function (that, field, isVisible) {
                return sprintf('<div class="input-group input-daterange"><input type="text" class="form-control date-filter-control bootstrap-table-filter-control-%s" style="width: 100%; visibility: %s"><div class="input-group-addon">to</div><input type="text" class="form-control date-filter-control bootstrap-table-filter-control-%s" style="width: 100%; visibility: %s"></div>', field, isVisible, field, isVisible);
            }
    ...
    BootstrapTable.prototype.onColumnSearch = function (event) {
        ...
        //=====删除=====
        //this.onSearch(event);  删除，就是这句导致的冲突，filter的search覆盖了table的search
        //=====新增=====
        this.initSearch();
        this.updatePagination();
